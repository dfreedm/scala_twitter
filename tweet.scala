...
class Tweet {
	...
	def parse(json:List[Any]):Unit = {
		json.foreach(tweetpart => {
			tweetpart match{
				case tp:Tuple2[String,Any] => {
					tp._1 match {
						case "user" => {parseUser(tp._2.asInstanceOf[List[Any]])}
	...
	def parseUser(user_list:List[Any]):Unit = {
		user_list.foreach(part => {
			part match{
				case p:Tuple2[String,Any] => {
					p._1 match {
